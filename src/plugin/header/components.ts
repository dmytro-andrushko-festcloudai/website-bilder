...
export default (editor: Editor, opts: RequiredPluginOptions) => {
  ...
  const idLogo = `${id}-logo`;
  const idNavMenu = `${id}-nav-menu`;
  const idLngMenu = `${id}-lng-menu`;
    ...
    model: {
      defaults: {
        ...
        components: [
          ...
          {
            attributes: {
              class: `${navbarPfx}-items-c lng-items`,
              "data-gjs": "navbar-items",
            },
            components: { type: idLngMenu },
          },
        ],
      },
    },
  ...
  Components.addType(idLogo, {
    model: {
      defaults: {
        extend: 'image',
        name: "Navbar Menu",
        tagName: "img",
        attributes: { class: `${navbarPfx}-logo` },
        content: `<img src=${logoUrl} alt="" style="height: 40px" />`,
      },
    },
  });
  ...
  Components.addType(idLngMenu, {
    model: {
      defaults: {
        ...
        attributes: { class: `${navbarPfx}-menu` },
        components: [
          { type: idNavMenuLink, components: "uk" },
          { type: idNavMenuLink, components: "en" },
        ],
      },
    },
  });
